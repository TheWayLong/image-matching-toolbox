...
def img2tensor(im, device,color='gray'):
    if color=='gray':
        return torch.from_numpy(im/255.).float()[None, None].to(device)
    elif color=='RGB' :
...
def resize_im(wo, ho, imsize=None, dfactor=1, value_to_scale=min, enforce=False):
    wt, ht = wo, ho
    resize = imsize > 0
...
def read_im_gray(im_path, imsize=None, dfactor=1,value_to_scale=min):
    im, scale = read_im(im_path, imsize,dfactor,value_to_scale)
    return im.convert('L'), scale

def load_gray_scale_tensor(im_path, device, imsize=None, dfactor=1,value_to_scale=min):
...
def load_im_tensor(im_path, device, imsize=None, normalize=True,
                   with_gray=False, raw_gray=False, dfactor=1,value_to_scale=min):
    im_rgb, scale = read_im(im_path, imsize, dfactor=dfactor,value_to_scale=value_to_scale)
